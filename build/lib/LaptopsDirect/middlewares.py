...
#Rotating proxies middleware, modified from: https://github.com/TeamHG-Memex/scrapy-rotating-proxies
...
class BanDetectionMiddleware(object):
    ...
    def _load_policy(cls, crawler):
        policy_path = crawler.settings.get(
            ...
            'LaptopsDirect.policy.BanDetectionPolicy'
        )
...
#Middleware to utilise a proxy server, must be before the httpproxy middleware in the config otherwise it gets ignored. 
from w3lib.http import basic_auth_header
import base64

class CustomProxyMiddleware(object):
    def process_request(self, request, spider):
        #request.meta['proxy'] = "http://85.10.51.63:80"
        request.meta['proxy'] = "140.227.211.47:8080"
        #request.meta['proxy'] = "69.197.181.202:3128" - replaced old reliable 09/11/21
        #request.meta['proxy'] = "159.65.69.186:9300" -previous old reliable
        #request.meta['proxy'] = "194.35.233.128:89"
        #request.headers['Proxy-Authorization'] = basic_auth_header('JLa7RhT1J1JGKGAmJzdaRAhb', 'DJu4xGMRtDHsTiQWXMPrivvM')
        # Use the following lines if your proxy requires authentication
        #proxy_user_pass = "JLa7RhT1J1JGKGAmJzdaRAhb:DJu4xGMRtDHsTiQWXMPrivvM"22
        # setup basic authentication for the proxy
        #encoded_user_pass = base64.b64encode(proxy_user_pass)
        #request.headers['Proxy-Authorization'] = 'Basic ' + encoded_user_pass

class CustomProxyMiddleware2(object):
    def process_request(self, request, spider):
        request.meta['proxy'] = "140.227.66.105:58888"
        
from scrapy import signals


class LaptopsdirectSpiderMiddleware(object):
...
class LaptopsdirectDownloaderMiddleware(object):
    # Not all methods need to be defined. If a method is not defined,
...
#Middleware to exclude OcUK B-Grade items
class OcUKBGRADEExclude(object):

    def process_request(self, request, spider):
        if "/b-grade-" in request.url:
...
#Middleware to exclude OcUK Archive items
class OcUKArchiveExclude(object):
